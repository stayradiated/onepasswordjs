...
Crypto =
  ...
  pbkdf2: (password, salt, iterations=10000, keySize=512) ->
    ...
    # TODO: See if ezcrypto is faster
    # https://github.com/ElmerZhang/ezcrypto/blob/master/lib/pbkdf2.js
    ...
    class hmacSHA512
      ...
      encrypt: (sjclArray) ->
        ...
        hex = nodeCrypto.createHmac('sha512', @key).update(buffer).digest('hex')
        bits = sjcl.codec.hex.toBits(hex)
        return bits

    salt = sjcl.codec.hex.toBits(@toHex(salt))
    bits = sjcl.misc.pbkdf2(password, salt, iterations, keySize, hmacSHA512)
